...
def consolidate_csv(
    ...
    syscall_filename = os.path.join(
        csvs_dir_path, f"{fsync}_{request_count}_syscalls.csv"
    )
    syscall_times_filename = os.path.join(
        csvs_dir_path, f"{fsync}_{request_count}_syscalls-times.csv"
    )
...
def run_all_tasks(r, process, request_count):
    ...
    with open(consolidated_csv_path, "w", newline="") as consolidated_csv:
        ...
        for fsync in ["everysec", "no"]:
            ...
            run_benchmark(request_count, csvs_dir_path, 6380, fsync, save_csv=False)
            stop_event.set()
            monitor_thread.join()
            ...
            if not headers_written:
                csv_filename = os.path.join(
                    csvs_dir_path, f"{fsync}_{request_count}.csv"
                )
