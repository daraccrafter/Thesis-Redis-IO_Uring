...
if __name__ == "__main__":
    ...
    for count in request_counts:
        for fsync in appendfsync_configs:
            filename_pattern_syscalls = f"{count}_syscalls_run{{iteration}}.csv"
            filename_pattern_syscall_times = (
                f"{count}_syscalls-times_run{{iteration}}.csv"
            )
            average_syscalls_files(
                csvs_dir_path,
                iterations,
                filename_pattern_syscalls,
                f"{count}_syscalls_avg.csv",
            )
            average_syscall_times_csv_files(
                csvs_dir_path,
                iterations,
                filename_pattern_syscall_times,
                f"{count}_syscalls-times_avg.csv",
            )
    stop_server(process)
