...
output_dir_redis_io_uring_sqpoll, output_dir_redis_io_uring, graphs_dir, _, _ = create_directories(
...
if __name__ == "__main__":
    for i in range(1, iterations + 1):
        ...
        for count in request_counts:
            ...
            run_benchmark(count, output_dir_redis_io_uring_sqpoll, 6379, i)
        for count in request_counts:
            print(f"\t\tRunning benchmark with {count} requests on Redis IO_Uring...")
            run_benchmark(count, output_dir_redis_io_uring, 6380, i)

    for count in request_counts:
        filename_pattern = f"{count}_run{{iteration}}.csv"
        avg_redis_io_uring_sqpoll = average_rps_csv_files(
            output_dir_redis_io_uring_sqpoll, iterations, filename_pattern, f"{count}_avg.csv"
        )
