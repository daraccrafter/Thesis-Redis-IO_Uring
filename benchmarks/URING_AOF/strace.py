...
if __name__ == "__main__":
    ...
    for i in range(1, iterations + 1):
        for fsync in appendfsync_configs:
            ...
            for count in request_counts:
                strace_proc = run_strace(process.pid,count,csvs_dir_path,log_dir_path,i,fsync)
                run_benchmark(count, csvs_dir_path, 6379, i,fsync, False)
    ...
    for count in request_counts:
        for fsync in appendfsync_configs:
            ...
            filename_pattern_syscall_times = f"{fsync}_{count}_syscalls-times_run{{iteration}}.csv"
            average_syscalls_files(csvs_dir_path,iterations,filename_pattern_syscalls,f"{fsync}_{count}_syscalls_avg.csv")
            average_syscall_times_csv_files(csvs_dir_path,iterations,filename_pattern_syscall_times,f"{fsync}_{count}_syscalls-times_avg.csv")
    stop_server(process)
